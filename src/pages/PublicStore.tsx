  ...
  return (
    <div className={`min-h-screen ${theme.light} pb-32`}>
      ...
      <header className={`bg-gradient-to-r ${theme.gradient} text-white shadow-md sticky top-0 z-20 overflow-hidden transition-all duration-300`}>
        ...
        <div className={`max-w-4xl mx-auto px-4 md:px-8 relative z-10 flex transition-all duration-300 ${isScrolled ? 'py-3 flex-row items-center gap-4' : 'py-6 md:py-8 flex-col md:flex-row items-center gap-6 text-center md:text-left'}`}>
          ...
          <div className={`flex-1 flex transition-all duration-300 ${isScrolled ? 'flex-row items-center justify-between' : 'flex-col'}`}>
            <div className={`${isScrolled ? 'text-left' : ''}`}>
              <h1 className={`font-black drop-shadow-sm transition-all duration-300 ${isScrolled ? 'text-xl md:text-2xl mb-0 line-clamp-1' : 'text-3xl md:text-4xl mb-2'}`}>{store.name}</h1>
              {!isScrolled && (
                <p className="text-white/90 text-lg font-medium animate-in fade-in slide-in-from-top-2">Escolha seus produtos favoritos e faça seu pedido!</p>
              )}
            </div>
            ...
            {store.phone && (
              <a 
                ...
                className={`inline-flex items-center gap-2 bg-white/20 hover:bg-white/30 rounded-full font-medium backdrop-blur-sm transition-all duration-300 ${isScrolled ? 'px-3 py-2 md:px-4 md:py-2 text-sm' : 'mt-4 px-4 py-2 text-sm'}`}
              >
                ...
                {!isScrolled && <span className="hidden sm:inline">Falar no WhatsApp</span>}
              </a>
            )}
          </div>
        </div>
